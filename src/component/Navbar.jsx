...
export default function Navbar(){
    ...
    const {isRectangle, setIsRectangle} = useContext(ThemeContext);
    ...
    return(
        <nav className={`${theme} ${!isRectangle? "fixed bg-transparent":"static dark:bg-slate-700 bg-indigo-300"} ${menuClick && !isRectangle? "h-[100vh]":"h-auto" } top-0 right-0 left-0 z-50 backdrop-blur-2xl ${lang =="Ar"? "font-[ArabicFont]":"font-[LatinFont]"} border-b-4 dark:border-indigo-300 border-slate-700 grid grid-cols-6 ${menuClick && "grid-rows-3"} items-center py-4`}>
            ...
            {isMobile ? (
                menuClick && (
                  <motion.ul
                    ...
                    className={`list-none lg:col-span-3 col-span-6 lg:order-2 order-5 lg:justify-center`}
                  >
                    {mySiteInfo[lang].nav.map((e,index)=><motion.li variants={ulItem} key={e} className={`flex py-2 justify-center ${lang =="Fr"|| lang =="En"? "xl:text-base lg:text-sm text-3xl":"xl:text-xl lg:text-base text-3xl"}`}><a href={`#${mySiteInfo["En"].nav[index]}`} className={` lg:drop-shadow-none drop-shadow-[0_0_0.5px_black] mx-4 dark:text-gray-300 text-white hover:text-slate-700 dark:hover:text-white transition-all font-bold`} onClick={()=>setMenuClick((prev)=>false)}>{e}</a></motion.li>)}
                  </motion.ul>
                )
            ) : (
